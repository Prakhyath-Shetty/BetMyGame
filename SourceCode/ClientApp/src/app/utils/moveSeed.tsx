 ...
 */
export function setColours(state: any, action: any) {
 ...
 */
export function findSeedGroup(state: any, seedId: any) {
  let house;
 ...
 */
function getSeedPosition(state: any, seedId: any) {
 ...
 */
function seedRemainingMoves(state: any, seedId: any) {
  const seedGroup = findSeedGroup(state, seedId);
 ...
 */
export function getLudoSeeds(gameData: any) {
  let seeds = {};
 ...
 */
function checkForWins(seedGroup: any, dispatch: any) {
  ...
  dispatch(
    sendNotification({
      type: "success",
      title: "Congrats",
      message: `House ${[number]} has finished.`,
    })
  );
}
...
function killSeed(
  ...
  const seeds: any = getLudoSeeds(state);
  const seedsInSamePosition = Object.keys(seeds).filter((seed) => {
    return seed !== movingSeed && seeds[seed].position === seedPosition;
  });

  const seedId =
    seedsInSamePosition && seedsInSamePosition.length > 0
      ? seedsInSamePosition[0].toString()
      : null;
 ...
 */
function makeSeedMove(options: any) {
  const {
    lastMove,
    state,
    dispatch,
...
function hasAlternativeMove(seedGroup: any) {
  const positions: any = [];
  const movesLeft: any = [];
  ...
  const moves = store
    ...
    .gameData.dieResult.map((result: any) => result.value);
 ...
 */
function invalidPlay(state: any, seedId: any, moves: any, dispatch: any) {
  const position = getSeedPosition(state, seedId);
  const movesLeft = seedRemainingMoves(state, seedId);
  const movesSum = moves.reduce((a: any, b: any) => Number(a) + Number(b), 0);
 ...
 */
export function setSeedPosition(data: any) {
  ...
  if (moves.length > 0) {
    const move = moves.reduce((a: any, b: any) => Number(a) + Number(b), 0); // sum up the content of the array.
